...
after(async function () {
  const parallel = []
  parallel.push(moveFile(`${process.cwd()}/config`, `${process.cwd()}/test/config`))
  parallel.push(removeFile(`${process.cwd()}/ui`))
  parallel.push(removeFile(`${process.cwd()}/api`))
  parallel.push(removeFile(`${process.cwd()}/assets`))
  await Promise.all(parallel)
})
