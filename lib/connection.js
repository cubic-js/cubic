...
class Connection {
     ...
     * Quite a messy algorithm. Might be changed in the future. If you wanna contribute, that's a great way to do it.
     */
    throttle() {
        return new Promise((resolve, reject) => {
    ...
    errCheck(res, verb, query) {
        return new Promise((resolve, reject) => {
            ...
            if(res) statusPrefix = res.statusCode ? (res.statusCode.toString().split(''))[0] : null

            // Response not 2xx?
            if (res && statusPrefix !== 2) {
