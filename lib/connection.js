...
class Connection {
    ...
    setup(options) {
        return new Promise((resolve, reject) => {
             ...
             */
            if (options.ignore_limiter) this.delay_diff = 0
            else if (options.user_key && options) this.delay_diff = 275
            else this.delay_diff = 525
    ...
    throttle() {
        return new Promise((resolve, reject) => {
            ...
            let delay = this.delay_ongoing * this.delay_diff - diff + next_int
            ...
            // Otherwise, wait for delay difference
                ...
                setTimeout(() => {
                    ...
                    if (curr_int > next_int) this.throttle()
     ...
     */
    delay(delay) {
        this.delay_next_int = new Date().getTime() + delay
    ...
    errCheck(res, verb, query) {
        return new Promise((resolve, reject) => {
            ...
            if (res.statusCode !== 200) {
                ...
                // Rate Limited (with access-token gained)
                else if (this.access_token && res.statusCode === 429) {
                    this.delay(5000)
                        .then(() => this.request(verb, query))
                        .then((res) => resolve(res))
                }

                // No Access token & rate limited
                else if (!this.access_token && res.statusCode === 429) {
                    this.delay(10000)
                        .then(() => this.request(verb, query))
                        .then((res) => resolve(res))
