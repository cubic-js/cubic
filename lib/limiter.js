...
class Limiter {
  async check (req, endpoint) {
    ...
    return new Promise(resolve => {
      ...
      limit(req.user.uid + endpoint.route, (err, time, actions) => this.limit(err, time, actions))
    })
  }
    ...
    // Limit Rate if necessary
      ...
      if (actions > 0) {
        err = {
          ...
          reason: `Request intervals too close. You need to wait ${time + 100}ms to continue.`
        }
      } else {
        err = {
          error: 'Rate limit exceeded.',
          reason: `Max requests per interval reached. You need to wait ${time + 100}ms to continue.`
        }
      }
      this.resolve(err)
    }
