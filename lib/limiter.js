...
class Limiter {
  constructor() {
    this.redis = Redis.createClient({ url: blitz.config[blitz.id].redisUrl })
  }
  ...
  async check (req, endpoint) {
    ...
    return new Promise(resolve => {
      ...
      limit(req.user.uid + endpoint.route, (err, time, actions, resolve) => this.limit(err, time, actions, resolve))
    })
  }
